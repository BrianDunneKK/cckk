...
config = {
    "num_camera_images": 30,  # Number of images to capture from camera (if camera is available)
    "camera_image_delay_secs": 12,  # Delay in seconds between camera captures
    "output_file_path": "result.txt",
    "image_path": "local_only/Earth1/",
    "angle_range": 5,  #5  # degrees ... 5 or None for no angle filtering
    "distance_outlier_percentiles": (10,90),  # tuple ... (10,90) or None for no outlier distance filtering
    "min_inc_matches": 5,  # Minimum number of included matches for analysis to be considered valid (None for default minimum)
    "speed_outlier_percentiles": None #(10,90),  # tuple ... (10,90) or None for no outlier speed filtering
}
...
for i in range(num_images):
    ...
    if cckkCamera.cckkCameraORB.available():
        sleep(config['camera_image_delay_secs'])

logger.info(f"Execute match analysis and calculate speed for {num_images-1} image pairs")
...
for i in range(num_images-1):
    ...
    matcher.analyse(angle_range=config['angle_range'], outlier_percentiles=config["distance_outlier_percentiles"])

    analysis = cckkCV.cckkMatchAnalysis(img1.info, img2.info, matcher.info)
    str = calculate_speed.add_analysis(analysis, config['min_inc_matches'])
    if str is not None:
...
speed_km_per_sec, result_analysis = calculate_speed.result(outlier_percentiles=config['speed_outlier_percentiles'])
...
logger.info(f"Detailed Analysis: Speeds Excluded={result_analysis['speeds_excluded']}, " +
            f"Speed Limits={result_analysis['speed_limits'][0]:.4f} to {result_analysis['speed_limits'][1]:.4f} km/sec")
